...
func verifyAndUpServer(checker AliveChecker, errCtx interface{}) {
	...
	go PingServer(AliveChecker, errCtx, errCh)
...
func handleAddServer(s *models.Server) error {
	...
	if (s.Type == models.SERVER_TYPE_OFFLINE) {
		log.Infof("try return slave %+v", slave)
		err = callHttp(nil, genUrl(*apiServer, "/api/server_group/", slave.GroupId, "/addServer"), "POST", s)
		if err != nil {
			log.Errorf("do return slave %v failed %v", slave, errors.ErrorStack(err))
			return err
		}
	}
